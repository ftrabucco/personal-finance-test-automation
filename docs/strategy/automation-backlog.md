...
- [x] Validated the full fix set (#37-#41) against real staging: all `scheduled-generation.destructive.spec.ts` tests (`CF-SCH-GEN-001` through `009`) and the full `@destructive` suite pass — 29/30, the one failure being the pre-existing, already-known `BUG-2026-005` (unrelated to this work).
  - **Constraint found while validating: the scheduled-generation spec must run with a single worker (`--workers=1`) against any shared environment (staging, prod).** `GET /gastos/generate` isn't scoped to one test's entities — it processes *every* pending item for the authenticated user in one call. Running these tests in parallel (e.g. plain `npm run test:staging:destructive`, which doesn't force `--workers=1` the way `test:staging:destructive:ui` does) lets one worker's `generate()` call sweep up another worker's not-yet-asserted/not-yet-cleaned-up entities, causing cross-test failures and orphaned data whose `finally` block never got a chance to run (a test that fails before capturing a generated gasto's id can't clean it up). Confirmed directly: a parallel `test:staging:destructive` run against staging left 9 orphaned gastos + 1 orphaned tarjeta from cross-test interference; the same suite run serially left nothing behind. Always run `tests/api/scheduled-generation.destructive.spec.ts` (and `tests/ui/scheduled-generation.destructive.spec.ts`) with `--workers=1` against staging/prod: e.g. `TEST_ENV=staging ALLOW_DESTRUCTIVE_TESTS=true npx playwright test tests/api/scheduled-generation.destructive.spec.ts --project=api --workers=1`.
  - Also cleaned up unrelated leftover staging data found during validation: 6 orphaned gastos (~$6000 ARS) from an earlier environment mix-up in an unrelated session (this repo's `.env` defaults `TEST_ENV=staging`, so an unqualified local run can silently hit real staging — see the `test:staging:preflight`/environment docs), plus the cross-test-interference leftovers above. Not a product bug, just test-run hygiene.
